"""
P4-PL-2026Q2-v2-AllA — Phase 4 V2 实盘策略（全A样本，standalone）
================================================================================
聚宽【策略回测】环境直接粘贴运行，无外部依赖。

spec_id: P4-PL-2026Q2-v2-AllA
...
V2 改进（相对 P1 Baseline）:
  ...
  2. F5 overlay：全市场 F5 z-score < -0.3 时额外减仓 20%（V1 阈值 -0.5 触发太少）
  3. V4 激进版仓位管理：7%/10%/15% 回撤阈值 + 25% 止损 + 5% 恢复 + 63 交易日冷却
  4. 成本模型：万三+印花+千一滑点（与 V2 研究脚本一致）
...
仓位管理（V4 激进版，每日检查）:
  ...
  回撤 > 10% → 仓位 50%
  回撤 > 15% → 仓位 30%
  回撤 > 25% → 止损，仓位 20%，进入 63 交易日冷却
  从低点反弹 5% → 恢复加仓
  F5 z-score < -0.3 → 额外减仓 20%
...
def compute_target_position(context, nav_history, f5_z_current,
    ...
    # V6 最终方案：去掉大盘择时（所有择时方案都降低Sharpe）
    # 只保留回撤约束作为风控（不预测市场，只看策略自身回撤）
    # 回撤<15%满仓，保留完整收益；回撤>15%才降仓，自适应风控

    # 仓位上下限
    target_w = max(min(target_w, POSITION_CEIL), POSITION_FLOOR)
