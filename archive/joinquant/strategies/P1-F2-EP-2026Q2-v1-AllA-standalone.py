"""
P1-F2-EP-2026Q2-v1-AllA — EP 当期因子策略（全A样本，standalone）
====================================================================
聚宽策略编辑器直接粘贴运行，无外部依赖。

spec_id: P1-F2-EP-2026Q2-v1-AllA
样本: 全A股（剔除ST/次新股/金融股）
基准: 000985.XSHG（中证全指）
...
因子定义（Phase 1.2，IC 验证通过）:
  ep_spot = net_profit / (market_cap * 1e8)
  即市盈率倒数（盈利收益率），越高=越便宜=好

选股方式:
  - 按 ep_spot 降序排序
  - 取前 50 只
  - 权重按 ep_spot 横截面 z-score 归一化（IC加权，非简单等权）
...
Gate 1 结论（2026-06-27，详见 research/decisions/P1-F1-EV-2026Q2-v1-decision.md）:
  - 市值中性化后 IC t=3.41（大幅过门槛）
  - 行业中性化后 IC t=2.94（大幅过门槛）
  - Q分组有倒挂（成长溢价），故用全截面IC加权而非Q5多头

口径来源: research/decisions/P1-F1-EV-2026Q2-v1-decision.md Phase 1.2 终判
"""
...
# ============================================================
# 五、F2 因子计算（EP 当期，Phase 1.2 IC 验证通过）
# ============================================================

def calculate_ep_factor(df):
    """计算 EP 当期因子及过滤指标。

    Phase 1.2 口径（IC 验证：市值中性化 t=3.41，行业中性化 t=2.94）:
      ep_spot = net_profit / (market_cap * 1e8)
      即市盈率倒数（盈利收益率），越高=越便宜=好
...
def factor_rebalance(context):
    """季频因子选股：5/9/11 月，计算 IC 加权目标权重，存入 g.target_weights。"""
    ...
    # 4. 因子计算
    df = calculate_ep_factor(df)

    # 5. 过滤：净利润>0 + 资产负债率<=1 + EP 有效
    mask = df['net_profit'] > 0
    mask &= df['debt_to_assets'] <= 1.0
    mask &= df['ep_spot'].notna()
    df = df[mask]

    if df.empty:
        log.info('[%s] 无符合条件的 EP 股票' % date_str)
        return

    # 6. IC加权选股：按 ep_spot 降序取前 N，权重按 z-score 归一化
    df = df.sort_values('ep_spot', ascending=False).head(g.stock_num)

    ep_vals = df['ep_spot'].values
    z = (ep_vals - ep_vals.mean()) / (ep_vals.std() if ep_vals.std() > 0 else 1)
    ...
    for code, w in list(g.target_weights.items())[:5]:
        log.info('  买入 %s  ep=%.4f  w=%.2f%%' % (
            code, df.loc[code, 'ep_spot'], w * 100))
