"""
P8-F3replaceF2-F5F6F3-strategy — CST-Quant P8 F3替换F2诊断
================================================================================
...
诊断动机
================================================================================
P2 仅凭 FM 回归不显著（t=1.48）剔除 F3 股息率，但：
  - F3 IC 已通过（市值中性化 t=2.74），是唯一"IC 通过却被主观剔除"的因子
  - P2 自己承认"FM 检验 ≠ 组合价值"（F5 是反例：FM t=0.90 但组合收益翻倍）
  - 这是一次"假阴性"诊断：验证 F3 在组合内是否有正贡献

F3 数据源: finance.STK_XR_XD（除权除息表手算，非 valuation.dividend_ratio）
F3 处理: NaN 填 0，不作为过滤，同样做市值中性化 + winsorize
================================================================================
来源: P4-PL-2026Q2-v2-AllA-F2F5F6-baseline，经 P5 F6-MOM 40d 优化 + 跨样本验证
因子: F3(Dividend Yield) 替换 F2(EP) + F5(LowVol) + F6(MOM-40d)，等权 Z-score（F2/F3 共线，替换诊断）
...
回测结果（2014-01-01 ~ 2026-06-28，聚宽策略环境）:
  【待回测】P8 四因子诊断版本，对照 P5-final（777%/Sharpe 0.67）
  判定标准: Sharpe ≥ 0.67 且收益 ≥ 777% → F3 有益；否则 F3 归档
...
================================================================================
  ...
  F3-DIV: dividend_yield = sum(每股分红, 过去400天) / 不复权收盘价
          （数据源: finance.STK_XR_XD 除权除息表手算）

合成方式（四因子等权 z-score，无参数避免过拟合）:
  combined = (1/4) * F2_z + (1/4) * F5_z + (1/4) * F6_z + (1/4) * F3_z
...
后续优化方向
================================================================================
  - 四因子等权是简单方案，后续可考虑 ICIR 加权或 Risk Parity
...
# 聚宽策略环境：finance 需显式 import（研究环境自动注入，策略环境不注入）
try:
    from jqdata import finance
except ImportError:
    pass  # 聚宽策略环境直接注入，无需额外操作
...
# F3 股息率参数
DIV_LOOKBACK_DAYS = 400      # F3 分红回溯窗口（400天，约1.3年）
DIV_BATCH_SIZE = 300         # 分红查询分批大小（绕过 run_query 行数限制）
...
def calc_dividend_yield(date_str, stocks):
    """计算股息率因子（F3-DIV）。

    数据源: finance.STK_XR_XD（聚宽除权除息表）
      - valuation.dividend_ratio 在聚宽策略环境中不可用，需手算

    计算: dividend_yield = sum(每股分红, 过去400天) / 不复权收盘价
      - 每股分红 = bonus_ratio_rmb / 10（表中为每10股派息）
      - 只统计已实施分红（plan_progress == "实施方案"）
      - 不复权收盘价: get_price(..., fq=None)
    ...
    query_date = pd.Timestamp(date_str)
    start_date = (query_date - pd.Timedelta(days=DIV_LOOKBACK_DAYS)).strftime('%Y-%m-%d')
    ...
    for i in range(0, len(stocks), DIV_BATCH_SIZE):
        ...
        try:
            q = query(
                ...
                finance.STK_XR_XD.a_xr_date <= date_str,
                finance.STK_XR_XD.plan_progress == '实施方案',
            )
            df_div = finance.run_query(q)
            if df_div is None or df_div.empty:
                continue
            df_div['div_per_share'] = df_div['bonus_ratio_rmb'] / 10.0
            agg = df_div.groupby('code')['div_per_share'].sum()
            for code, total_div in agg.items():
                div_map[code] = float(total_div)
        except Exception as e:
            log.info('[F3] 分红查询异常（批次 %d）: %s' % (i // DIV_BATCH_SIZE + 1, str(e)))
            continue
    ...
    try:
        df_px = get_price(
            list(div_map.keys()), end_date=date_str, count=1,
            fields=['close'], skip_paused=False, panel=False, fq=None
        )
        if df_px is None or df_px.empty:
            return {}
        if 'code' in df_px.columns:
            close_series = df_px.set_index('code')['close']
        else:
            return {}
    except Exception as e:
        log.info('[F3] 收盘价获取异常: %s' % str(e))
        return {}

    result = {}
    for code, total_div in div_map.items():
        close_px = close_series.get(code) if hasattr(close_series, 'get') else None
        if close_px is None or np.isnan(close_px) or close_px <= 0:
            continue
        dy = total_div / float(close_px)
        if np.isfinite(dy):
            result[code] = dy

    log.info('[F3] 股息率计算完成: %d 只股票有分红数据' % len(result))
    return result
...
def calculate_combined_factors(df, vol_map, mom_map, div_map):
    ...
    # F3 原始因子（NaN 填 0，不分红=0 股息率=低分）
    df['F3_div_raw'] = df.index.map(lambda c: div_map.get(c, 0.0))
    return df
...
def apply_neutralization(df):
    ...
    # F3 市值中性化 + winsorize
    f3_neut = neutralize_ols(df['F3_div_raw'].values, log_mcap.values)
    df['F3_div'] = winsorize_cross_section(pd.Series(f3_neut, index=df.index))
    return df
...
def factor_rebalance(context):
    """季频四因子选股：5/9/11 月，EP+LowVol+MOM+Dividend 等权 z-score 合成 + 流动性过滤。"""
    ...
    log.info('=' * 50)
    log.info('[%s] 季度调仓开始（F2+F5+F6+F3 四因子）' % date_str)
    ...
    # 4. 因子计算
    div_map = calc_dividend_yield(date_str, list(df.index))
    df = calculate_combined_factors(df, vol_map, mom_map, div_map)
    ...
    # F3 替换 F2：只保留 F3+F5+F6 三因子（F2 和 F3 共线，替换而非叠加）
    df = df.dropna(subset=['F3_div', 'F5_vol', 'F6_mom'])
    if len(df) < 30:
        log.info('[%s] 中性化后样本不足: %d 只' % (date_str, len(df)))
        return

    # 7. 三因子等权 z-score 合成（F3 替换 F2）
    f3_std = df['F3_div'].std()
    f5_std = df['F5_vol'].std()
    f6_std = df['F6_mom'].std()
    df['F3_z'] = (df['F3_div'] - df['F3_div'].mean()) / (f3_std if f3_std > 0 else 1)
    df['F5_z'] = (df['F5_vol'] - df['F5_vol'].mean()) / (f5_std if f5_std > 0 else 1)
    df['F6_z'] = (df['F6_mom'] - df['F6_mom'].mean()) / (f6_std if f6_std > 0 else 1)
    # 等权合成（1/3 + 1/3 + 1/3）
    df['combined'] = (1.0 / 3.0) * df['F3_z'] + (1.0 / 3.0) * df['F5_z'] + (1.0 / 3.0) * df['F6_z']
    ...
    for code, w in list(actual_weights.items())[:5]:
        log.info('  买入 %s  ep=%.4f vol=%.4f mom=%.4f div=%.4f avg_money=%.0f万 w=%.2f%%' % (
            code, df.loc[code, 'ep_spot'], df.loc[code, 'vol_60d'],
            df.loc[code, 'mom_12_1'], df.loc[code, 'F3_div_raw'], df.loc[code, 'avg_money'] / 1e4, w * 100))
    if len(actual_weights) > 5:
        log.info('  ... 共 %d 只' % len(actual_weights))

    rebalance_ordered(context, actual_weights)
    log.info('[%s] 季度调仓完成（F2+F5+F6+F3 四因子）' % date_str)
