"""
P9-F2F5F6-industry-neutral-strategy — CST-Quant 行业中性化策略
================================================================================
CST = Cheap · Stable · Trending（低估 · 稳定 · 趋势）
聚宽【策略回测】环境直接粘贴运行，单文件无外部依赖。

来源: P5-F2F5F6-40d-final-strategy，新增行业中性化（行业内 z-score）
因子: F2(EP) + F5(LowVol) + F6(MOM-40d)，行业内等权 Z-score
...
改进动机（P5 → P9）
================================================================================
P5 三因子策略全样本 Sharpe 0.67、累计 777%，但因子合成采用横截面 z-score，
意味着因子得分包含了行业间系统性差异。例如：
  - 某行业整体 EP 偏高（银行/地产），横截面 z-score 会系统性高估该行业股票
  - 某行业整体波动率偏低（公用事业），F5 会系统性偏好该行业
  - 组合行业暴露由因子行业属性被动决定，而非主动选择

行业中性化的目的：去除因子中的行业均值效应，使每只股票的因子得分仅反映
"相对同行业 peers 的相对优劣"，而非"相对全市场的绝对优劣"。

================================================================================
行业中性化方案（行业内 z-score）
================================================================================
P5 原版（横截面 z-score）:
  ep_z  = (F2_ep - mean_all) / std_all
  combined = (1/3)*ep_z + (1/3)*vol_z + (1/3)*mom_z

P9 新版（行业内 z-score）:
  ep_z  = (F2_ep - mean_industry) / std_industry    （申万一级行业分组）
  combined = (1/3)*ep_z + (1/3)*vol_z + (1/3)*mom_z

中性化链路（与 P5 一致 + 新增行业层）:
  1. 市值中性化: F2/F5/F6 raw → OLS 残差（去除 log(mcap) 线性影响）[继承 P5]
  2. winsorize:  1%/99% 分位数裁剪（去极值）[继承 P5]
  3. 行业中性化: 横截面 z-score → 行业内 z-score（去除行业均值）[P9 新增]
     - 分组键: 申万一级行业 sw_l1
     - 每个因子在行业内独立标准化
     - 行业内样本 < 2 时退化为该组均值去心（std=0 → 除1）

行业分类: 申万一级行业（sw_l1），与 exclude_finance_stocks 一致。
金融股已在股票池阶段剔除，剩余约 27 个一级行业。
...
验证目标（P9 vs P5）
================================================================================
  1. 全样本 Sharpe 不显著劣化（P5 基准 0.67）
  2. 行业集中度下降：组合行业分布更均衡（HHI 下降）
  3. 分段回测不出现新的失效段
  4. 若 Sharpe 提升，说明行业暴露此前在拖累因子表现

================================================================================
后续优化方向
================================================================================
  - 行业内 z-score 是因子层中性化，未约束组合层行业权重（选股仍按 combined 全局排序）
  - 后续可做组合层行业中性（每行业等权配额），但会牺牲选股自由度
  - 可对比"行业 demean + 横截面 z"vs"行业内 z"两种中性化路径
"""
...
# 行业中性化（P9 核心改动）
# True=行业内 z-score（申万一级行业分组，去除行业均值效应）
# False=横截面 z-score（退回 P5 原版行为）
INDUSTRY_NEUTRALIZE = True
INDUSTRY_SCHEME = 'sw_l1'    # 申万一级行业分类方案
...
def get_industry_series(stocks, date_str, scheme=INDUSTRY_SCHEME):
    """获取股票的行业分类，返回 pd.Series(index=code, values=industry_name)。

    用于行业中性化分组。取不到行业的股票归为 '未知'。
    scheme 默认 sw_l1（申万一级行业），与 exclude_finance_stocks 一致。
    """
    if not stocks:
    ...
    try:
        ...
        ind = {}
    if not ind:
        return pd.Series({c: '未知' for c in stocks})
    mapping = {}
    for code, schemes in ind.items():
        if not isinstance(schemes, dict):
            mapping[code] = '未知'
        ...
        if isinstance(entry, dict):
            name = str(entry.get('industry_name', '') or '')
            mapping[code] = name if name else '未知'
        else:
            mapping[code] = '未知'
    # 未出现在 ind 中的股票也补 '未知'
    for c in stocks:
...
def factor_rebalance(context):
    """季频三因子选股：5/9/11 月，EP+LowVol+MOM 行业内 z-score 合成 + 流动性过滤。

    P9 改动：z-score 由横截面改为行业内（申万一级行业分组），实现行业中性化。
    """
    ...
    log.info('=' * 50)
    log.info('[%s] 季度调仓开始（F2+F5+F6 三因子·行业中性化）' % date_str)
    ...
    # 6.5 行业分类（P9 行业中性化所需，申万一级行业 sw_l1）
    df['industry'] = get_industry_series(list(df.index), date_str)
    n_ind = df['industry'].nunique()
    log.info('[%s] 行业分类: %d 个申万一级行业，%d 只股票' % (
        date_str, n_ind, len(df)))

    # 7. 三因子等权 z-score 合成（P9: 行业内 z-score，行业中性化）
    if INDUSTRY_NEUTRALIZE:
        # 行业内 z-score：每只股票的因子值相对同行业 peers 标准化，
        # 去除行业均值效应，使因子得分仅反映行业内相对优劣。
        def _ind_zscore(s):
            std = s.std()
            return (s - s.mean()) / (std if std > 0 else 1)
        df['F2_z'] = df.groupby('industry')['F2_ep'].transform(_ind_zscore)
        df['F5_z'] = df.groupby('industry')['F5_vol'].transform(_ind_zscore)
        df['F6_z'] = df.groupby('industry')['F6_mom'].transform(_ind_zscore)
    else:
        # P5 原版横截面 z-score（INDUSTRY_NEUTRALIZE=False 时退回）
        f2_std = df['F2_ep'].std()
    ...
    for code, w in list(actual_weights.items())[:5]:
        log.info('  买入 %s [%s]  ep=%.4f vol=%.4f mom=%.4f avg_money=%.0f万 w=%.2f%%' % (
            code, df.loc[code, 'industry'], df.loc[code, 'ep_spot'], df.loc[code, 'vol_60d'],
            df.loc[code, 'mom_12_1'], df.loc[code, 'avg_money'] / 1e4, w * 100))
    if len(actual_weights) > 5:
        log.info('  ... 共 %d 只' % len(actual_weights))

    # 行业分布统计（P9 诊断：观察行业中性化效果）
    ind_dist = df['industry'].value_counts()
    top_inds = ', '.join('%s:%d' % (k, v) for k, v in ind_dist.head(5).items())
    log.info('[%s] 持仓行业分布(Top5): %s' % (date_str, top_inds))

    rebalance_ordered(context, actual_weights)
    log.info('[%s] 季度调仓完成（F2+F5+F6 三因子·行业中性化）' % date_str)
